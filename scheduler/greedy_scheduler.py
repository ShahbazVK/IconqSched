...
class GreedyScheduler(BaseScheduler):
    ...
    def ingest_query(
        ...
        if len(self.running_queries) == 0:
            ...
            prediction_len_per_query = future_len * (
                len(self.existing_query_concur_features) + 1
            )
            assert len(predictions) % prediction_len_per_query == 0 and len(
                predictions
            ) // prediction_len_per_query == len(self.queued_queries)
            all_score = []
            all_query_idx = []
            ...
            for i in range(len(self.existing_runtime_prediction)):
                ...
                weight = 1 - min(
                    (start_t - self.existing_start_time[i])
            ...
            for i in range(len(self.queued_queries)):
                ...
                old_existing_pred = np.asarray(
                    self.existing_runtime_prediction_adjusted
                )
                new_existing_pred = predictions[
                    (pred_idx + future_len) : (
                        pred_idx
                        + future_len
                        + future_len * len(self.existing_query_concur_features)
                    ) : future_len
                ]
                ...
                existing_benefits = np.sum(
                    delta_existing
                )  # * new_existing_pred_weight)
                ...
                for j in range(len(next_finish_idx_list)):
                    ...
                    delta = (
                        new_existing_pred - future_existing_pred
                    )  # * new_existing_pred_weight
                    if self.debug:
                        if self.logger:
                            self.logger.info(
                    ...
                    starve_penalty = (
                        max(next_finish_time - self.queued_queries_enter_time[i], 0.1)
                        * self.starve_penalty
                    )
